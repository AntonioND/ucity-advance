...
static const power_line_mask power_line_mask_table[] = {
    ...
    { 0b01011010, 0b01011010, T_POWER_LINES_TLRB },

    { 0b01011010, 0b01010010, T_POWER_LINES_TRB },
    { 0b01011010, 0b01011000, T_POWER_LINES_LRB },
    { 0b01011010, 0b01001010, T_POWER_LINES_TLB },
    { 0b01011010, 0b00011010, T_POWER_LINES_TLR },

    { 0b01011010, 0b01010000, T_POWER_LINES_RB },
    { 0b01011010, 0b00010010, T_POWER_LINES_TR },
    { 0b01011010, 0b01001000, T_POWER_LINES_LB },
    { 0b01011010, 0b00001010, T_POWER_LINES_TL },

    { 0b01011010, 0b01000000, T_POWER_LINES_TB },
    { 0b01011010, 0b00000010, T_POWER_LINES_TB },
    { 0b01011010, 0b01000010, T_POWER_LINES_TB },

    { 0b01011010, 0b00010000, T_POWER_LINES_LR },
    { 0b01011010, 0b00001000, T_POWER_LINES_LR },
    { 0b01011010, 0b00011000, T_POWER_LINES_LR },

    { 0b00000000, 0b00000000, T_POWER_LINES_LR }, // Default -> Always valid
};

// Checks if this is a building that needs electriciy
// Returns: TYPE_HAS_POWER or 0
uint16_t TypeBuildingHasElectricity(uint16_t type)
{
    type &= TYPE_MASK;
...
uint16_t TypeHasElectricityExtended(uint16_t type)
{
    if (type & TYPE_HAS_POWER)
        return TYPE_HAS_POWER; // Trivial case

    return TypeBuildingHasElectricity(type);
}
...
void MapTileUpdatePowerLines(int x, int y)
{
    ...
    if ((tile_type & TYPE_HAS_POWER) == 0)
        return; // Not a power line

    if ((tile_type & TYPE_MASK) == TYPE_WATER)
        return; // This is a bridge, don't update

    // If there are roads and train tracks, power lines can't be built
    if ((tile_type & (TYPE_HAS_ROAD | TYPE_HAS_TRAIN)) ==
        (TYPE_HAS_ROAD | TYPE_HAS_TRAIN))
        return;
    ...
    //
    // 1 = has power line, 0 = doesn't have power line.
    //
    // 0 1 2
    // 3 . 4 <- Bit order
    // 5 6 7
    ...
    if (TypeHasElectricityExtended(aux_tile_type) == TYPE_HAS_POWER)
    {
        if (aux_tile != T_POWER_LINES_LR_BRIDGE)
            flags |= 1 << 1;
    }
    ...
    if (TypeHasElectricityExtended(aux_tile_type) == TYPE_HAS_POWER)
    {
        if (aux_tile != T_POWER_LINES_TB_BRIDGE)
            flags |= 1 << 3;
    }
    ...
    while (1)
    {
        ...
        if ((plm->mask & flags) == plm->expected_result)
        {
            // Draw resulting tile
            CityMapDrawTile(plm->resulting_tile, x, y);
            return;
        }
...
{
    MapTileUpdatePowerLines(x - 1, y - 1);
    MapTileUpdatePowerLines(x, y - 1);
    MapTileUpdatePowerLines(x + 1, y - 1);

    MapTileUpdatePowerLines(x - 1, y);
    MapTileUpdatePowerLines(x, y);
    MapTileUpdatePowerLines(x + 1, y);
...
void MapDrawPowerLines(int force, int x, int y)
{
    ...
    if ((tile_type & TYPE_MASK) == TYPE_WATER)
    {
        ...
        int length = 0;
        int direction;
        CityMapCheckBuildBridge(force, x, y, TYPE_HAS_POWER, &length, &direction);
        ...
        int endx = x;
        int endy = y;
        CityMapBuildBridge(force, &endx, &endy, TYPE_HAS_POWER, length, direction);

        MapUpdateNeighboursPowerLines(x, y);
        MapTileUpdatePowerLines(endx, endy);

        return;
    }

    // Single power line tile
    // ----------------------
    ...
    {
        // Build road
        CityMapDrawTile(T_POWER_LINES_TB, x, y);
        goto built;
    }
